...
- **Follow the Lance error pattern**: every error variant carries `#[snafu(implicit)] location: Location` for automatic file/line tracking. Use `#[snafu(visibility(pub))]` on error enums to generate public context selectors. Use `BoxedError` (`Box<dyn Error + Send + Sync>`) for flexible source wrapping. See `storage/src/error.rs` for the canonical example.
